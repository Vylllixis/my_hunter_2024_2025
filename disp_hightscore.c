...
int imple_hsc_2(int sc, csfml_t csfml)
{
    if (sc == 5) {
...
int imple_hsc(int sc, csfml_t csfml)
{
    if (sc == 0) {
    ...
    imple_hsc_2(sc, csfml);
    return 0;
}

int print_hsc(int x, int y, int sc, csfml_t csfml)
{
...
int display_hightscore(csfml_t csfml)
{
    int a = 232;
    int b = 208;
    int score = csfml.hightscore;

    for (int i = 0; score > 9; i++) {
        print_hsc(a, b, (score % 10), csfml);
